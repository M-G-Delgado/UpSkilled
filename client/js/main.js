const CourseLoader = {
  ...
  init() {
    ...
    if (spinner) {
      ...
      console.log('Spinner displayed, starting fetch...');
    } else {
      console.error('Loading spinner element (#loadingSpinner) not found in DOM');
    }
    ...
    tryFetchCourses(3000)
      .then(data => {
        ...
        if (spinner) {
          ...
          console.log('Spinner hidden, initialization complete');
        } else {
          console.log('Spinner not found, initialization complete without hiding spinner');
        }
      })
      .catch(error => {
        console.error("Fetch error:", error);
        if (spinner) {
          spinner.textContent = "Unable to load courses. CORS issue, server down, or no data exists. Check console for details.";
          spinner.style.display = "block";
          console.log('Courses failed to load, spinner updated with error message');
        } else {
          console.log('Courses failed to load, no spinner to update');
        }
      });
